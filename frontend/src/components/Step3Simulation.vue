  ...
  <div class="simulation-panel">
    ...
    <div class="control-bar">
      <div class="status-group">
        <!-- Twitter Progresso della piattaforma -->
        <div class="platform-status twitter" :class="{ active: runStatus.twitter_running, completed: runStatus.twitter_completed }">
          ...
          <!-- Richieste di azione disponibili -->
        ...
        <!-- Reddit Progresso della piattaforma -->
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'Inizio...' : 'Inizia a generare un rapporto sui risultati' }} 
          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- CREATE_COMMENT: Lascia un commento -->
                ...
                <!-- Fallback universale: tipo o contenuto sconosciuto ma non gestito in precedenza -->
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // Il numero massimo di round passati dal passaggio 2
  minutesPerRound: {
    type: Number,
    default: 30 // L'impostazione predefinita è 30 minuti per round
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const phase = ref(0) // 0: Non iniziato, 1: Correre, 2: Completato
const isStarting = ref(false)
const isStopping = ref(false)
const startError = ref(null)
const runStatus = ref({})
const allActions = ref([]) // Tutte le azioni (cumulative incrementali）
const actionIds = ref(new Set()) // Raccolta dell'ID azione utilizzata per la deduplicazione
const scrollContainer = ref(null)

// Computed
// Mostra le azioni in ordine cronologico (le più recenti alla fine, cioè in basso）
...
// Tempo trascorso per la simulazione del formato (calcolato in termini di round e minuti per round）
...
// TwitterTempo trascorso simulato per la piattaforma
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// RedditTempo trascorso simulato per la piattaforma
...
// Ripristina tutto lo stato (per riavviare la simulazione）
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
  ...
  stopPolling()  // Interrompere i sondaggi che potrebbero esistere prima
}
...
const doStartSimulation = async () => {
  ...
  // Reimpostare prima tutti gli stati per assicurarsi che non siano influenzati dall'ultima simulazione
  resetAllState()
  
  isStarting.value = true
  startError.value = null
  addLog('Avvio della simulazione parallela a doppia piattaforma...')
  ...
  try {
    const params = {
      ...
      platform: 'parallel',
      force: true,  // forzare il riavvio
      enable_graph_memory_update: true  // Abilita aggiornamenti dinamici delle mappe
    }
    
    if (props.maxRounds) {
      params.max_rounds = props.maxRounds
      addLog(`Imposta il numero massimo di round di simulazione: ${props.maxRounds}`)
    }
    
    addLog('La modalità di aggiornamento dinamico delle mappe è stata attivata')
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Puliti i vecchi registri di simulazione, riavviata la simulazione')
      }
      addLog('✓ Il motore di simulazione è stato avviato correttamente')
      ...
      startError.value = res.error || 'Avvio non riuscito'
      addLog(`✗ Avvio non riuscito: ${res.error || 'errore sconosciuto'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(`✗ Eccezione di avvio: ${err.message}`)
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleStopSimulation = async () => {
  ...
  addLog('Interruzione della simulazione...')
  ...
  try {
    ...
    addLog(`interrompere l'eccezione: ${err.message}`)
  } finally {
    isStopping.value = false
  }
}
...
// Tieni traccia dell'ultimo round di ciascuna piattaforma per rilevare modifiche e registri di output
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Rileva le modifiche apportate su ciascuna piattaforma separatamente e genera i log
      ...
      // Rileva se la simulazione è stata completata (tramite runner_status Oppure la piattaforma completa il giudizio sullo status）
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Controllo extra: se il backend non è stato ancora aggiornato runner_status，Ma la piattaforma ha segnalato il completamento
      // Rilevamento del passaggio twitter_completed e reddit_completed Giudizio sullo stato
      const platformsCompleted = checkPlatformsCompleted(data)
      
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ Rilevato che tutte le simulazioni della piattaforma sono terminate')
        }
    ...
    console.warn('Impossibile ottenere lo stato di esecuzione:', err)
  }
}

// Controlla se tutte le piattaforme abilitate sono state completate
const checkPlatformsCompleted = (data) => {
  // Se non ci sono dati sulla piattaforma, ritorna false
  if (!data) return false
  
  // Controlla lo stato di completamento di ciascuna piattaforma
  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true
  
  // Se almeno una piattaforma è stata completata, controlla se tutte le piattaforme abilitate sono state completate
  // passa actions_count Determinare se la piattaforma è abilitata (se count > 0 o correre era true）
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted
  
  // Se nessuna piattaforma è abilitata, ritorna false
  if (!twitterEnabled && !redditEnabled) return false
  
  // Verificare che tutte le piattaforme abilitate siano state completate
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // Utilizzare all_actions Ottieni l'elenco completo delle azioni
      const serverActions = res.data.all_actions || []
      
      // Aggiungi in modo incrementale nuove azioni (rimuovi la duplicazione）
      ...
      // Nessuno scorrimento automatico, consentendo agli utenti di visualizzare liberamente la sequenza temporale
      // Le nuove azioni verranno aggiunte in fondo
    }
  } catch (err) {
    console.warn('Impossibile ottenere lo stato dettagliato:', err)
  }
}
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('La richiesta di generazione del report è stata inviata, attendere....')
    return
  }
  
  isGeneratingReport.value = true
  addLog('Avvio della generazione del report...')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ Attività di generazione del report avviata: ${reportId}`)
      
      // Vai alla pagina del report
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`✗ Impossibile avviare la generazione del report: ${res.error || 'errore sconosciuto'}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ Avvia l'eccezione per la generazione del report: ${err.message}`)
    isGeneratingReport.value = false
  }
}
...
onMounted(() => {
  addLog("Step3 Inizializzazione dell'esecuzione della simulazione")
  if (props.simulationId) {
    doStartSimulation()
  }
})
