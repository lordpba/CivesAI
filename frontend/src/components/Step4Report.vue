            ...
            >
              ...
              <div class="section-body" v-show="!collapsedSections.has(idx)">
                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  ...
                  <span class="loading-text">Generazione{{ section.title }}...</span>
                </div>
              </div>
            </div>
          </div>
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (La generazione del contenuto è completata, ma l'intero capitolo potrebbe non essere ancora completato) -->
                  ...
                  <!-- Section Complete (Generazione del capitolo completata) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- Quando la risposta è definitiva, viene visualizzato un messaggio speciale -->
...
const toggleRawResult = (timestamp, event) => {
  // Salva la posizione del pulsante rispetto alla finestra
  ...
  // Dopo aver atteso l'aggiornamento del DOM, regola la posizione di scorrimento per mantenere il pulsante nella stessa posizione
...
const toggleSectionCollapse = (idx) => {
  // È possibile comprimere solo i capitoli completati
  if (!generatedSections.value[idx + 1]) return
...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // Icona della lampadina: rappresenta l'intuizione
  },
  'panorama_search': {
  ...
  'interview_agents': {
    ...
    color: 'green',
    icon: 'users' // Icona utente: rappresenta una conversazione
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // Icona del fulmine: rappresenta la velocità
  },
  'get_graph_statistics': {
    name: 'Graph Stats',
    color: 'cyan',
    icon: 'chart' // Icona del grafico: rappresenta le statistiche
  },
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // Icona del database: rappresenta l'entità
  }
}
...
const parseInsightForge = (text) => {
  ...
  try {
    // Problemi di analisi degli estratti
    const queryMatch = text.match(/Analizzare il problema:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Estrai scenari di previsione
    const reqMatch = text.match(/Scenari di previsione:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    // Estrai statistiche - Partita"Fatti di previsione rilevanti: XArticolo"Formato
    const factMatch = text.match(/Fatti di previsione rilevanti:\s*(\d+)/)
    const entityMatch = text.match(/Enti coinvolti:\s*(\d+)/)
    const relMatch = text.match(/catena di relazioni:\s*(\d+)/)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])
    
    // Estrai sottoproblemi: estrazione completa, nessun limite al numero
    const subQSection = text.match(/### Sottoproblemi analitici\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
      const lines = subQSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.subQueries = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }
    
    // Estrai i fatti chiave: estrazione completa, nessun limite alla quantità
    const factsSection = text.match(/### 【fatti chiave】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (factsSection) {
    ...
    // Estrai entità principali: estrazione completa, incluso riepilogo e conteggio dei fatti correlati
    const entitySection = text.match(/### 【entità centrale】\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
      const entityText = entitySection[1]
      // premere "- **" Dividi blocchi solidi
      const entityBlocks = entityText.split(/\n(?=- \*\*)/).filter(b => b.trim().startsWith('- **'))
      result.entities = entityBlocks.map(block => {
        const nameMatch = block.match(/^-\s*\*\*(.+?)\*\*\s*\((.+?)\)/)
        const summaryMatch = block.match(/Sommario:\s*"?(.+?)"?(?:\n|$)/)
        const relatedMatch = block.match(/Fatti rilevanti:\s*(\d+)/)
        return {
    ...
    // Catena di relazioni di estrazione: estrazione completa, nessun limite alla quantità
    const relSection = text.match(/### 【catena di relazioni】\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    // Estrai query
    const queryMatch = text.match(/Domanda:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Estrai statistiche
    const nodesMatch = text.match(/Numero totale di nodi:\s*(\d+)/)
    const edgesMatch = text.match(/numero totale di bordi:\s*(\d+)/)
    const activeMatch = text.match(/fatti validi attuali:\s*(\d+)/)
    const histMatch = text.match(/storia\/fatto scaduto:\s*(\d+)/)
    ...
    // Estrai fatti attualmente validi: estrazione completa, nessun limite alla quantità
    const activeSection = text.match(/### 【fatti validi attuali】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
      const lines = activeSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.activeFacts = lines.map(l => {
        // Rimuovere numeri e virgolette
        const factText = l.replace(/^\d+\.\s*/, '').replace(/^"|"$/g, '').trim()
        return factText
      }).filter(Boolean)
    }
    
    // Estrai fatti storici/scaduti: estrazione completa, nessun limite di quantità
    const histSection = text.match(/### 【storia\/fatto scaduto】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (histSection) {
    ...
    // Estrai le entità coinvolte: estrazione completa, nessun limite alla quantità
    const entitySection = text.match(/### 【Enti coinvolti】\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    // Estrarre gli argomenti dell'intervista
    const topicMatch = text.match(/\*\*Argomenti dell'intervista:\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()
    
    // Estrarre il numero delle persone intervistate (es. "5 / 9 simulazione di bitAgent"）
    const countMatch = text.match(/\*\*Numero di interviste:\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    // Estrarre le ragioni per la selezione degli intervistati
    const reasonMatch = text.match(/### Ragioni per la selezione degli intervistati\n([\s\S]*?)(?=\n---\n|\n### Trascrizione dell'intervista)/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }
    
    // Analizzare le ragioni della scelta di ciascuna persona
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Formato1: numeri. **nome（index=X）**：Motivo
        // Ad esempio: 1. **Ex-alunni_345（index=1）**：Come alunno dell'Università di Wuhan...
        ...
        // Formato2: - Selezionare il nome (indice X): Motivo
        // Ad esempio: - Seleziona genitore_601（index 0）：In qualità di rappresentante del gruppo genitore...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*Seleziona([^（(]+)(?:[（(]index\s*=?\s*\d+[)）])?[：:]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        // Formato3: - **Nome (indice X)**: Motivo
        // Ad esempio: - **genitori_601（index 0）**：In qualità di rappresentante del gruppo genitore...
        if (!headerMatch) {
        ...
        if (name) {
          ...
          // iniziare nuove persone
          currentName = name
          currentReason = reasonStart ? [reasonStart.trim()] : []
        } else if (currentName && line.trim() && !line.match(/^Non selezionato|^Per riassumere|^scelta finale/)) {
          // continuazione delle ragioni (esclusi i commi conclusivi）
          currentReason.push(line.trim())
        }
      }
      
      // Motivi per salvare l'ultima persona non identificata
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    // Estrai la trascrizione di ciascuna intervista
    const interviewBlocks = text.split(/#### intervista #\d+:/).slice(1)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Estrarre il titolo (ad esempio "studente"、"professionista dell'educazione" Aspetta）
      ...
      if (nameRoleMatch) {
        ...
        // Imposta il motivo della selezione della persona
        interview.selectionReason = individualReasons[interview.name] || ''
      }
      
      // Introduzione all'estrazione
      const bioMatch = block.match(/_Introduzione:\s*([\s\S]*?)_\n/)
      if (bioMatch) {
        interview.bio = bioMatch[1].trim().replace(/\.\.\.$/, '...')
      }
      
      // Estrai l'elenco delle domande
      const qMatch = block.match(/\*\*Q:\*\*\s*([\s\S]*?)(?=\n\n\*\*A:\*\*|\*\*A:\*\*)/)
      if (qMatch) {
        const qText = qMatch[1].trim()
        // Dividi le domande per numero
        const questions = qText.split(/\n\d+\.\s+/).filter(q => q.trim())
        if (questions.length > 0) {
          // Se la prima domanda è preceduta da"1."，Richiede una gestione speciale
      ...
      // Estrai risposte: punta a Twitter eReddit
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*Citazioni chiave|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // Separate Twitter and Reddit answers
        const twitterMatch = answerText.match(/【TwitterRisposta della piattaforma】\n?([\s\S]*?)(?=【RedditRisposta della piattaforma】|$)/)
        const redditMatch = answerText.match(/【RedditRisposta della piattaforma】\n?([\s\S]*?)$/)
        ...
        // Logica di fallback della piattaforma (compatibile con il vecchio formato: caso in cui è presente un solo tag della piattaforma）
        if (!twitterMatch && redditMatch) {
          // Solo risposta Reddit, copiata solo per essere visualizzata per impostazione predefinita quando non è testo segnaposto
          if (interview.redditAnswer && interview.redditAnswer !== '（La piattaforma non ha ricevuto risposta）') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '（La piattaforma non ha ricevuto risposta）') {
            interview.redditAnswer = interview.twitterAnswer
          }
        } else if (!twitterMatch && !redditMatch) {
          // Nessun contrassegno di sottopiattaforma (formato molto vecchio), nel complesso risposta
          interview.twitterAnswer = answerText
        }
      }
      
      // Estrai virgolette chiave (compatibile con più formati di citazione）
      const quotesMatch = block.match(/\*\*Citazioni chiave:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // corrispondenza delle priorità > "text" Formato
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // fallback: corrispondenza > "text" o > \u201Ctext\u201D（Virgolette cinesi）
        if (!quoteMatches) {
    ...
    // Estrai stralci di interviste
    const summaryMatch = text.match(/### Sintesi dell'intervista e punti chiave\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const queryMatch = text.match(/query di ricerca:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Numero di risultati estratti
    const countMatch = text.match(/trovato\s*(\d+)\s*Articolo/)
    if (countMatch) result.count = parseInt(countMatch[1])
    
    // Estrai fatti rilevanti: estrazione completa, nessun limite alla quantità
    const factsSection = text.match(/### Fatti rilevanti:\n([\s\S]*)$/)
    if (factsSection) {
      const lines = factsSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.facts = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }
    
    // Prova a estrarre informazioni secondarie (se presenti）
    const edgesSection = text.match(/### Bordi rilevanti:\n([\s\S]*?)(?=\n###|$)/)
    if (edgesSection) {
    ...
    // Prova a estrarre le informazioni sul nodo (se presenti）
    const nodesSection = text.match(/### Nodi correlati:\n([\s\S]*?)(?=\n###|$)/)
    if (nodesSection) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-header' }, [
        ...
        props.result.simulationRequirement && h('div', { class: 'header-scenario' }, [
          h('span', { class: 'scenario-label' }, 'Scenari di previsione: '),
      ...
      h('div', { class: 'insight-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `memoria della chiave corrente (${props.result.facts.length})`)
        ]),
        h('button', {
          class: ['insight-tab', { active: activeTab.value === 'entities' }],
          onClick: () => { activeTab.value = 'entities' }
        }, [
          h('span', { class: 'tab-label' }, `entità centrale (${props.result.entities.length})`)
        ]),
        h('button', {
          class: ['insight-tab', { active: activeTab.value === 'relations' }],
          onClick: () => { activeTab.value = 'relations' }
        }, [
          h('span', { class: 'tab-label' }, `catena di relazioni (${props.result.relations.length})`)
        ]),
        props.result.subQueries.length > 0 && h('button', {
          ...
          h('span', { class: 'tab-label' }, `sottoproblema (${props.result.subQueries.length})`)
        ])
      ]),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Ultimi fatti chiave associati nella memoria temporale'),
            h('span', { class: 'panel-count' }, `totale ${props.result.facts.length} Articolo`)
          ]),
          ...
          }, expandedFacts.value ? `chiudere ▲` : `Espandi tutto ${props.result.facts.length} Articolo ▼`)
        ]),
        
        // Entities Tab
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'entità centrale'),
            h('span', { class: 'panel-count' }, `totale ${props.result.entities.length} un`)
          ]),
          h('div', { class: 'entities-grid' },
            (expandedEntities.value ? props.result.entities : props.result.entities.slice(0, 12)).map((entity, i) => 
              h('div', { class: 'entity-tag', key: i, title: entity.summary || '' }, [
                ...
                entity.relatedFactsCount > 0 && h('span', { class: 'entity-fact-count' }, `${entity.relatedFactsCount}Articolo`)
              ])
            )
          ),
          ...
          }, expandedEntities.value ? `chiudere ▲` : `Espandi tutto ${props.result.entities.length} un ▼`)
        ]),
        
        // Relations Tab
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'catena di relazioni'),
            h('span', { class: 'panel-count' }, `totale ${props.result.relations.length} Articolo`)
          ]),
          ...
          }, expandedRelations.value ? `chiudere ▲` : `Espandi tutto ${props.result.relations.length} Articolo ▼`)
        ]),
        
        // Sub-queries Tab
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Sottoproblema di analisi della generazione di query di deriva'),
            h('span', { class: 'panel-count' }, `totale ${props.result.subQueries.length} un`)
          ]),
        ...
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'Nessuna entità centrale ancora'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'Nessun collegamento di relazione ancora')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Memoria attualmente valida (${props.result.activeFacts.length})`)
        ]),
        h('button', {
          class: ['panorama-tab', { active: activeTab.value === 'historical' }],
          onClick: () => { activeTab.value = 'historical' }
        }, [
          h('span', { class: 'tab-label' }, `memoria storica (${props.result.historicalFacts.length})`)
        ]),
        h('button', {
          class: ['panorama-tab', { active: activeTab.value === 'entities' }],
          onClick: () => { activeTab.value = 'entities' }
        }, [
          h('span', { class: 'tab-label' }, `Enti coinvolti (${props.result.entities.length})`)
        ])
      ]),
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Memoria attualmente valida'),
            h('span', { class: 'panel-count' }, `totale ${props.result.activeFacts.length} Articolo`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'Al momento non è presente memoria valida'),
          props.result.activeFacts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedActive.value = !expandedActive.value }
          }, expandedActive.value ? `chiudere ▲` : `Espandi tutto ${props.result.activeFacts.length} Articolo ▼`)
        ]),
        
        // Historical Facts Tab
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'memoria storica'),
            h('span', { class: 'panel-count' }, `totale ${props.result.historicalFacts.length} Articolo`)
          ]),
            ...
            (expandedHistorical.value ? props.result.historicalFacts : props.result.historicalFacts.slice(0, INITIAL_SHOW_COUNT)).map((fact, i) => 
              h('div', { class: 'fact-item historical', key: i }, [
                ...
                h('div', { class: 'fact-content' }, [
                  // Prova a estrarre informazioni sull'ora [time - time]
                  (() => {
          ...
          ) : h('div', { class: 'empty-state' }, 'Nessuna memoria storica ancora'),
          props.result.historicalFacts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedHistorical.value = !expandedHistorical.value }
          }, expandedHistorical.value ? `chiudere ▲` : `Espandi tutto ${props.result.historicalFacts.length} Articolo ▼`)
        ]),
        
        // Entities Tab
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Enti coinvolti'),
            h('span', { class: 'panel-count' }, `totale ${props.result.entities.length} un`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'Nessuna entità coinvolta ancora'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Mantieni lo stato di selezione della piattaforma indipendente per ciascuna coppia domanda-risposta
    const platformTabs = reactive({}) // { 'agentIdx-qIdx': 'twitter' | 'reddit' }
    
    // Ottieni la selezione attuale della piattaforma per una domanda
    const getPlatformTab = (agentIdx, qIdx) => {
      const key = `${agentIdx}-${qIdx}`
      return platformTabs[key] || 'twitter'
    }
    
    // Imposta la selezione della piattaforma per una domanda
    ...
    // Controlla se si tratta di testo segnaposto della piattaforma
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '（La piattaforma non ha ricevuto risposta）' || t === '(La piattaforma non ha ricevuto risposta)' || t === '[Nessuna risposta]'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "domandaX：" o "domandaX:" （Formato cinese, nuovo formato backend）
      // 2. "1. " o "\n1. " （Numero + punto, compatibile vecchio formato）
      let matches = []
      let match

      // Prova prima "domandaX：" Formato
      const cnPattern = /(?:^|[\r\n]+)domanda(\d+)[：:]\s*/g
      ...
      // Se non viene trovata alcuna corrispondenza, tornare a "numeri." Formato
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // Se non viene trovato alcun numero o ne viene trovato solo uno, restituire l'intero
      if (matches.length <= 1) {
    ...
    // Ottieni la risposta a una domanda
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // La suddivisione ha esito positivo e l'indice è valido
      if (answers.length > 1 && qIdx < answers.length) {
        return answers[qIdx] || ''
      }

      // Divisione non riuscita: la prima domanda restituisce la risposta completa, il resto restituisce il vuoto
      return qIdx === 0 ? answer : ''
    }
    
    // Controlla se una domanda ha risposte a doppia piattaforma (filtra il testo segnaposto）
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Entrambe le piattaforme hanno risposte reali (non testo segnaposto) e contenuti diversi
    ...
    return () => h('div', { class: 'interview-display' }, [
      ...
      props.result.interviews.length > 0 && h('div', { class: 'interview-detail' }, [
        ...
        // Q&A Conversation Thread - stile di domande e risposte
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Pulsante Premiere di cambio piattaforma doppia (visualizzato solo quando è presente una vera e propria doppia Risposta della piattaforma）
                  ...
                  // Expand/Collapse Button（Il testo segnaposto non viene visualizzato）
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      h('div', { class: 'quicksearch-header' }, [
        ...
        props.result.query && h('div', { class: 'header-query' }, [
          h('span', { class: 'query-label' }, 'Cerca: '),
      ...
      showTabs.value && h('div', { class: 'quicksearch-tabs' }, [
        ...
        hasEdges.value && h('button', {
          ...
          h('span', { class: 'tab-label' }, `relazione (${props.result.edges.length})`)
        ]),
        hasNodes.value && h('button', {
          class: ['quicksearch-tab', { active: activeTab.value === 'nodes' }],
          onClick: () => { activeTab.value = 'nodes' }
        }, [
          h('span', { class: 'tab-label' }, `nodo (${props.result.nodes.length})`)
        ])
      ]),
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'Nessun risultato rilevante trovato'),
          props.result.facts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedFacts.value = !expandedFacts.value }
          }, expandedFacts.value ? `chiudere ▲` : `Espandi tutto ${props.result.facts.length} Articolo ▼`)
        ...
        activeTab.value === 'edges' && hasEdges.value && h('div', { class: 'edges-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `totale ${props.result.edges.length} Articolo`)
          ]),
        ...
        activeTab.value === 'nodes' && hasNodes.value && h('div', { class: 'nodes-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `totale ${props.result.nodes.length} un`)
          ]),
...
// Passaggio attualmente attivo (per il display superiore）
const activeStep = computed(() => {
  const steps = workflowSteps.value
  // Trova il passaggio attualmente attivo
  const active = steps.find(s => s.status === 'active')
  if (active) return active
  
  // Se non è attivo, torna all'ultimo passaggio eseguito
  const doneSteps = steps.filter(s => s.status === 'done')
  if (doneSteps.length > 0) return doneSteps[doneSteps.length - 1]
  
  // Altrimenti torna al primo passaggio
  return steps[0] || { noLabel: '--', title: 'In attesa di iniziare', status: 'todo', meta: '' }
})
...
const renderMarkdown = (content) => {
  ...
  // Rimuovere il titolo secondario all'inizio（## xxx），Perché il titolo del capitolo è già visualizzato nello strato esterno
  let processedContent = content.replace(/^##\s+.+\n+/, '')
  
  // Blocchi di codice di processo
  let html = processedContent.replace(/```(\w*)\n([\s\S]*?)```/g, '<pre class="code-block"><code>$2</code></pre>')
  
  // Gestire il codice in linea
  html = html.replace(/`([^`]+)`/g, '<code class="inline-code">$1</code>')
  
  // Titoli di elaborazione
  html = html.replace(/^#### (.+)$/gm, '<h5 class="md-h5">$1</h5>')
  html = html.replace(/^### (.+)$/gm, '<h4 class="md-h4">$1</h4>')
  html = html.replace(/^## (.+)$/gm, '<h3 class="md-h3">$1</h3>')
  html = html.replace(/^# (.+)$/gm, '<h2 class="md-h2">$1</h2>')
  
  // Gestire i blocchi tra virgolette
  html = html.replace(/^> (.+)$/gm, '<blockquote class="md-quote">$1</blockquote>')
  
  // Elenchi di elaborazione: supporta i sottoelenchi
  ...
  // Elenco wrapper non ordinato
  html = html.replace(/(<li class="md-li"[^>]*>.*?<\/li>\s*)+/g, '<ul class="md-ul">$&</ul>')
  // Elenco ordinato degli imballaggi
  html = html.replace(/(<li class="md-oli"[^>]*>.*?<\/li>\s*)+/g, '<ol class="md-ol">$&</ol>')

  // Pulisci tutto lo spazio bianco tra gli elementi dell'elenco
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // Pulisci lo spazio dopo il tag di inizio elenco
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Cancella lo spazio prima del tag di fine dell'elenco
  ...
  // Manipolazione dei divisori
  html = html.replace(/^---$/gm, '<hr class="md-hr">')
  
  // Gestione delle interruzioni di riga: le righe vuote diventano interruzioni di paragrafo, le interruzioni di riga singola diventano <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  
  // avvolto nel paragrafo
  html = '<p class="md-p">' + html + '</p>'
  
  // Pulisci i paragrafi vuoti
  html = html.replace(/<p class="md-p"><\/p>/g, '')
  html = html.replace(/<p class="md-p">(<h[2-5])/g, '$1')
  html = html.replace(/(<\/h[2-5]>)<\/p>/g, '$1')
  html = html.replace(/<p class="md-p">(<ul|<ol|<blockquote|<pre|<hr)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>|<\/pre>)<\/p>/g, '$1')
  // Clean up before and after block-level elements <br> etichetta
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // pulire <p><br> Following block-level elements (resulting from extra blank lines）
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Pulisci consecutivamente <br> etichetta
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Pulisci il tag di apertura del paragrafo immediatamente successivo all'elemento a livello di blocco. <br>
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Corretta la numerazione degli elenchi ordinati non consecutivi: quando articolo singolo <ol> Mantieni la numerazione incrementata quando separata dal contenuto del paragrafo
...
const getLogLevelClass = (log) => {
  if (log.includes('ERROR') || log.includes('Errore')) return 'error'
  if (log.includes('WARNING') || log.includes('avvertimento')) return 'warning'
  // INFO Utilizza i colori predefiniti e non contrassegnarli come success
  return ''
}
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          // section_complete - Generazione del capitolo completata
          if (log.action === 'section_complete') {
            if (log.details?.content) {
              generatedSections.value[log.section_index] = log.details.content
              // Espandi automaticamente i capitoli appena generati
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // La logica di scorrimento viene unificata ed elaborata in nextTick al termine del ciclo.
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // Se l'attività è completata, scorri verso l'alto；Altrimenti scorri fino in fondo per seguire gli ultimi post
            if (isComplete.value) {
...
// Estrai il contenuto della risposta finale: estrai il contenuto del capitolo dalla risposta LLM
const extractFinalContent = (response) => {
  if (!response) return null
  
  // Prova ad estrarre <final_answer> contenuto all'interno dei tag
  ...
  // provare a trovare Final Answer: Il seguente contenuto (supporta più formati)
  // formato1: Final Answer:\n\ncontenuto
  // formato2: Final Answer: contenuto
  const finalAnswerMatch = response.match(/Final\s*Answer:\s*\n*([\s\S]*)$/i)
  if (finalAnswerMatch) {
    return finalAnswerMatch[1].trim()
  }
  
  // Prova a trovare la risposta finale: Ciò che segue
  const chineseFinalMatch = response.match(/risposta finale[:：]\s*\n*([\s\S]*)$/i)
  if (chineseFinalMatch) {
    return chineseFinalMatch[1].trim()
  }
  
  // Se con ## o # o > All'inizio potrebbe trattarsi di contenuti con ribasso diretto
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // Se il contenuto è lungo e contiene un formato markdown, prova a rimuovere il processo di riflessione e ritorna
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Rimuovi Thought: Processo di pensiero all'inizio
...
/* Console Logs - con Step3Simulation.vue Sii coerente */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;
